...
print(holder.credentials["test-cred"][0].to_dict())

verifier = VerifierInstance()

# Create VP Request
VP_request = verifier.presentation_request(requested_attributes=["studentId", "name"])

# Send Forward VP Request
VP_response = holder.present_credential(vp_request=VP_request, vc_name="test-cred", always_hidden_keys=["secret"])
